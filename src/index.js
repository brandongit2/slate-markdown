...
function handler(md, pos, isInline) {
    ...
    switch (md[pos]) {
        ...
        case '#': {
            if (!isInline) { // Titles/Headers
                ...
                const html = `<h${numPounds}>${module.exports(innerHTML, true)}</h${numPounds}>`;
            ...
            if (!isInline) {
                ...
                for (let i = 0; i < els.length;) {
                    if (els[i][0] < currentIndentationLevel) {
                        ...
                        html += `<li>${module.exports(els[i][1], true)}</li>`;
                        i++;
                    }
                }
                ...
                return {
                    html: `<s>${module.exports(content, true)}</s>`,
        ...
        case '\\': {
            if (md.slice(pos + 1, pos + 7) === 'begin{') {
                ...
                switch (arg) {
                    case 'aside': {
                        const html = `<div class="aside">${module.exports(content)}</div>`;
                        return {
                            html,
                    ...
                    case 'labelledlist': {
                        ...
                        while (true) {
                            ...
                            html += `<p>${module.exports(md.slice(i, split), true)}</p>`;
                            html += `<p>${module.exports(md.slice(split + 3, eol), true)}</p>`;
        ...
        case '*': { // Bold
            if (isInline && md[pos + 1] === '*') {
                ...
                return {
                    html: `<b>${module.exports(content, true)}</b>`,
        ...
        case '_': { // Italics
            if (isInline && md[pos + 1] === '_') {
                ...
                return {
                    html: `<i>${module.exports(content, true)}</i>`,
    ...
    if (isInline) {
        ...
        const html = `<p>${module.exports(innerHTML, true)}</p>`;
